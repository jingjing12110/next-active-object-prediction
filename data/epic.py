...
class EpicDatasetV2(Dataset):
    ...
    def generate_img_mask_pair(self):
        import shutil
        import cv2
        save_path = '/media/kaka/HD2T/dataset/EPIC_KITCHENS/img_gt_val/'
        ...
        for index in self.data.index:
            ...
            nao_bbox = self.data.loc[index, 'nao_bbox']

            mask = self.generate_mask(img, nao_bbox)

            shutil.copy(img_file, os.path.join(save_path,
                                               f'imgs/{str(index).zfill(6)}.jpg'))
            cv2.imwrite(os.path.join(save_path,
                                     f'gts/{str(index).zfill(6)}.jpg'), 255*mask)
