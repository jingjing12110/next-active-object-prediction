...
class ConvLSTMCell(nn.Module):
    ...
    def init_hidden(self, batch_size, hidden, shape):
        if self.Wci is None:
            self.Wci = Variable(
                torch.zeros(1, hidden, shape[0], shape[1])).cuda(device=args.device_ids[0])
            self.Wcf = Variable(
                torch.zeros(1, hidden, shape[0], shape[1])).cuda(device=args.device_ids[0])
            self.Wco = Variable(
                torch.zeros(1, hidden, shape[0], shape[1])).cuda(device=args.device_ids[0])
        ...
        return (
        Variable(torch.zeros(batch_size, hidden, shape[0], shape[1])).cuda(device=args.device_ids[0]),
        Variable(torch.zeros(batch_size, hidden, shape[0], shape[1])).cuda(device=args.device_ids[0]))
...
if __name__ == '__main__':
    ...
    convlstm = ConvLSTM(input_channels=512,
                        hidden_channels=[128, 64, 64, 32, 32], kernel_size=3,
                        step=5,
                        effective_step=[4]).cuda(device=args.device_ids[0])
    convlstm = torch.nn.DataParallel(convlstm, device_ids=args.device_ids)
    loss_fn = torch.nn.MSELoss()

    input = Variable(torch.randn(1, 512, 64, 32)).cuda(device=args.device_ids[0])
    target = Variable(torch.randn(1, 32, 64, 32)).double().cuda(device=args.device_ids[0])

    output = convlstm(input)
    output = output[0][0].double()
    res = torch.autograd.gradcheck(loss_fn, (output, target), eps=1e-6,
