# @File : unet_vgg.py 
# @Time : 2019/8/9 
# @Email : jingjingjiang2017@gmail.com 

import torch
import torch.nn as nn
# import torch.nn.functional as F
from torchvision import models

# vgg16 = models.vgg16(pretrained=True)


class ConvBlock(nn.Module):
    """
    Helper module that consists of a Conv -> BN -> ReLU
    """
    def __init__(self, in_channels, out_channels, padding=1, kernel_size=3,
                 stride=1):
        super().__init__()
        self.conv = nn.Conv2d(in_channels, out_channels, padding=padding,
                              kernel_size=kernel_size, stride=stride)
        # self.bn = nn.BatchNorm2d(out_channels)
        self.relu = nn.ReLU()

    def forward(self, x):
        x = self.conv(x)
        # x = self.bn(x)
        x = self.relu(x)

        return x


class Bridge(nn.Module):
    """
    This is the middle layer of the UNet which just consists of some
    """
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.bridge = nn.Sequential(
            ConvBlock(in_channels, out_channels),
            ConvBlock(out_channels, out_channels)
        )

    def forward(self, x):
        return self.bridge(x)


class UpsampleBlock(nn.Module):
    """
    Up block that encapsulates one up-sampling step which consists of
    Upsample -> ConvBlock -> ConvBlock
    """
    def __init__(self, in_channels, out_channels,
                 up_conv_in_channels=None,
                 up_conv_out_channels=None,
                 upsampling_method='conv_transpose'):
        super(UpsampleBlock, self).__init__()

        if up_conv_in_channels is None:
            up_conv_in_channels = in_channels
        if up_conv_out_channels is None:
            up_conv_out_channels = out_channels

        if upsampling_method == 'conv_transpose':
            self.upsample = nn.ConvTranspose2d(up_conv_in_channels,
                                               up_conv_out_channels,
                                               kernel_size=2,
                                               stride=2)
        elif upsampling_method == 'bilinear':
            self.upsample = nn.Sequential(
                nn.Upsample(mode='bilinear', scale_factor=2),
                nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1)
            )

        self.conv_block_1 = ConvBlock(in_channels, out_channels)
        self.conv_block_2 = ConvBlock(out_channels, out_channels)

    def forward(self, up_x, down_x):
        """
        :param up_x: this is the output from the previous up block
        :param down_x: this is the output from the down block
        :return: upsampled feature map
        """
        x = self.upsample(up_x)
        x = torch.cat([x, down_x], 1)

        x = self.conv_block_1(x)
        x = self.conv_block_2(x)
        return x


class UpsampleDecoder(nn.Module):
    def __init__(self, in_channels, out_channels,
                 up_conv_in_channels=None,
                 up_conv_out_channels=None,
                 upsampling_method='conv_transpose'):
        super(UpsampleDecoder, self).__init__()

        if up_conv_in_channels is None:
            up_conv_in_channels = in_channels
        if up_conv_out_channels is None:
            up_conv_out_channels = out_channels

        if upsampling_method == 'conv_transpose':
            self.upsample = nn.ConvTranspose2d(up_conv_in_channels,
                                               up_conv_out_channels,
                                               kernel_size=2,
                                               stride=2)
        elif upsampling_method == 'bilinear':
            self.upsample = nn.Sequential(
                nn.Upsample(mode='bilinear', scale_factor=2),
                nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1)
            )

        self.conv_block_1 = ConvBlock(in_channels, out_channels)
        # self.conv_block_2 = ConvBlock(out_channels, out_channels)

    def forward(self, up_x):
        """
        :param up_x: this is the output from the previous up block
        :return: upsampled feature map
        """
        x = self.upsample(up_x)

        x = self.conv_block_1(x)
        # x = self.conv_block_2(x)
        return x


class UNetVGG16(nn.Module):
    def __init__(self, n_classes=2):
        super(UNetVGG16, self).__init__()
        self.DEPTH = 6

        vgg16 = models.vgg16(pretrained=True)
        # features = list(vgg16.features.children())

        down_blocks = []
        up_blocks = []

        down_blocks.append(nn.Sequential(*list(vgg16.features.children()))[:4])
        down_blocks.append(nn.Sequential(*list(vgg16.features.children()))[4:9])
        down_blocks.append(nn.Sequential(*list(vgg16.features.children()))[9:16])
        down_blocks.append(nn.Sequential(*list(vgg16.features.children()))[16:23])
        down_blocks.append(nn.Sequential(*list(vgg16.features.children()))[23:30])

        self.down_blocks = nn.ModuleList(down_blocks)
        del down_blocks

        # center
        self.pool = nn.Sequential(list(vgg16.features.children())[30])
        del vgg16
        self.bridge = Bridge(512, 512)

        # up_blocks
        up_blocks.append(UpsampleBlock(in_channels=512 + 256,
                                       out_channels=512,
                                       up_conv_in_channels=512,
                                       up_conv_out_channels=256))
        up_blocks.append(UpsampleBlock(in_channels=512 + 256,
                                       out_channels=256,
                                       up_conv_in_channels=512,
                                       up_conv_out_channels=256))
        up_blocks.append(UpsampleBlock(in_channels=256 + 256,
                                       out_channels=128,
                                       up_conv_in_channels=256,
                                       up_conv_out_channels=256))
        up_blocks.append(UpsampleBlock(in_channels=128 + 64,
                                       out_channels=64,
                                       up_conv_in_channels=128,
                                       up_conv_out_channels=64))
        up_blocks.append(UpsampleBlock(in_channels=64 + 64,
                                       out_channels=64,
                                       up_conv_in_channels=64,
                                       up_conv_out_channels=64))

        self.up_blocks = nn.ModuleList(up_blocks)
        del up_blocks

        self.out = nn.Conv2d(64, n_classes, kernel_size=1, stride=1)

    def forward(self, x):
        pre_pools = dict()
        pre_pools[f'layer_0'] = x

        for i, block in enumerate(self.down_blocks, start=1):
            x = block(x)
            pre_pools[f'layer_{i}'] = x

        # center
        x = self.bridge(self.pool(x))

        for i, block in enumerate(self.up_blocks, start=1):
            key = f'layer_{self.DEPTH - i}'
            x = block(x, pre_pools[key])

        x = self.out(x)
        del pre_pools

        return x


class VggEncoderDecoder(nn.Module):
    def __init__(self, n_classes=2):
        super(VggEncoderDecoder, self).__init__()
        self.DEPTH = 5

        down_blocks = []
        up_blocks = []
        vgg16 = models.vgg16(pretrained=True)

        down_blocks.append(nn.Sequential(*list(vgg16.features.children()))[:4])
        down_blocks.append(nn.Sequential(*list(vgg16.features.children()))[4:9])
        down_blocks.append(nn.Sequential(*list(vgg16.features.children()))[9:16])
        down_blocks.append(nn.Sequential(*list(vgg16.features.children()))[16:23])
        down_blocks.append(nn.Sequential(*list(vgg16.features.children()))[23:30])

        self.down_blocks = nn.ModuleList(down_blocks)
        del down_blocks

        # center
        self.pool = nn.Sequential(list(vgg16.features.children())[30])
        del vgg16
        self.bridge = Bridge(512, 512)

        # up_blocks
        # up_blocks.append(UpsampleDecoder(in_channels=512,
        #                                  out_channels=512,
        #                                  up_conv_in_channels=512,
        #                                  up_conv_out_channels=512))
        up_blocks.append(UpsampleDecoder(in_channels=512,
                                       out_channels=512,
                                       up_conv_in_channels=512,
                                       up_conv_out_channels=512))
        up_blocks.append(UpsampleDecoder(in_channels=512,
                                       out_channels=256,
                                       up_conv_in_channels=512,
                                       up_conv_out_channels=512))
        up_blocks.append(UpsampleDecoder(in_channels=256,
                                       out_channels=128,
                                       up_conv_in_channels=256,
                                       up_conv_out_channels=256))
        up_blocks.append(UpsampleDecoder(in_channels=64,
                                       out_channels=64,
                                       up_conv_in_channels=128,
                                       up_conv_out_channels=64))

        self.up_blocks = nn.ModuleList(up_blocks)
        # self.up_blocks = nn.Sequential(up_blocks)
        del up_blocks

        self.out = nn.Conv2d(64, n_classes, kernel_size=1, stride=1)

    def forward(self, x):
        # pre_pools = dict()
        # pre_pools[f'layer_0'] = x

        for i, block in enumerate(self.down_blocks, start=1):
            x = block(x)
            # pre_pools[f'layer_{i}'] = x

        # center
        # x = self.bridge(self.pool(x))

        for i, block in enumerate(self.up_blocks, start=1):
            x = block(x)

        x = self.out(x)
        # del pre_pools

        return x


if __name__ == '__main__':
    from torchsummary import summary
    # summary(your_model, input_size=(channels, H, W))
    # model = UNetVGG16()
    model = VggEncoderDecoder()
    # summary(model.cuda(), input_size=(3, 512, 512))
    summary(model.cuda(), input_size=(3, 224, 320))  # 被32整除 (3, 224, 384)
